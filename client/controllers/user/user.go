...
import (
	"im/client/controllers"
	"im/client/libs/define"
	"im/client/logic/userLogic"
	"im/client/models/userModel"
	"encoding/json"
)
...
func (u *UserController) Login() {
	...
	u.Ctx.SetCookie("user",user.UserName,86400, "/",  "localhost", "", false)
	u.Data["json"] = u.RenderData(code, msg, retData)
	u.ServeJSON()
}

func (u *UserController) GGim()  {
	username := u.Ctx.GetCookie("user")
	if username == "" {
		u.Ctx.Redirect(302,"/")
		return
	}
...
func (u *UserController) ExistEmail()  {
	var (
		user    userModel.User
		retData userLogic.ReturnInfo
	)
	...
	if code != 0 {
		u.Data["json"] = u.RenderDataSimple(code, "ok")
		u.ServeJSON()
		return
	}
	u.Data["json"] = u.RenderData(code, "ok", retData)
	u.ServeJSON()
}
